...
impl Verifier {
    ...
    fn check_output(
        ...
        let epk = match de_ct(jubjub::ExtendedPoint::from_bytes(epk)) {
            Some(p) => p,
            None => return false,
        };
...
impl BatchValidator {
    ...
    /// `BatchValidator` can continue to be used, but it may include some or all of the
    /// bundle's proofs and signatures.
    ///
